...
while cap.isOpened():
    ...
    if not monitoring_enabled:
        ...
        cv2.imshow("Monitor K3 - YOLO11s", frame_paused)
    ...
    for box in results[0].boxes:
        ...
        if cls in APD_CLASS_MAP:
            apd_boxes.append((APD_CLASS_MAP[cls], xyxy))
    ...
    def report_violation(tid, vtype, frame_img):
        if (tid, vtype) in pelanggar_tercatat:
            return
        ...
        try:
            ...
            if res_cloud.status_code == 200:
                ...
                pelanggar_tercatat.add((tid, vtype))
    ...
    for person_id in person_boxes:
        ...
        for apd_name in APD_CLASS_MAP.values():
            if state["missing_counts"][apd_name] >= MISSING_FRAMES_THRESHOLD and apd_name in state["ever"]:
                report_violation(person_id, f"attempt_remove_{apd_name}", frame)
        ...
        if state["age"] >= NEVER_WEAR_FRAMES and not any(apd in state["ever"] for apd in APD_CLASS_MAP.values()):
            report_violation(person_id, "not_wearing_any_apd", frame)
        else:
            ...
            for apd_name in APD_CLASS_MAP.values():
                if state["age"] >= NEVER_WEAR_FRAMES and apd_name not in state["ever"]:
                    report_violation(person_id, f"not_wearing_{apd_name}", frame)

    # Tampilkan hasil secara visual di layar laptop
    cv2.imshow("Monitor K3 - YOLO11s", results[0].plot())
    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'):
        break
    if key == ord('p'):
